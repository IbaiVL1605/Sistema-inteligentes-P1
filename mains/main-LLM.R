...
decompose.solution <- function(result, problem) {
  ...
  # Cuando no hay solución, result$node_final es NA (vector atómico),
  # no una lista — hacer $actions sobre él daría error.
  # Comprobamos primero que node_final es una lista válida.
  node_ok <- !is.null(result$node_final) &&
    !(is.atomic(result$node_final) &&
        length(result$node_final) == 1 &&
        is.na(result$node_final))
  
  if (!node_ok) return(list(latencia_ms = NA, coste_eur = NA))
  
  actions <- result$node_final$actions
  
  # Sin acciones → devolver NA
...
print.solution.table <- function(results_list, labels, problem) {
  ...
  for (i in seq_along(results_list)) {
    ...
    # Cuando no hay solución, res$node_final es NA (vector atómico).
    # Hay que comprobarlo antes de acceder a $actions.
    node_ok   <- !is.null(res$node_final) &&
      !(is.atomic(res$node_final) &&
          length(res$node_final) == 1 &&
          is.na(res$node_final))
    actions   <- if (node_ok) res$node_final$actions else NULL
    tiene_sol <- !is.null(actions) && length(actions) > 0
    pasos     <- if (tiene_sol) length(actions) else "-"
